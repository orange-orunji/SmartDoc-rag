...
- **Agent 端到端评测体系**：`app/eval_agent.py` 五分类评测集（知识库/工具动作/联网/记忆/社交各 3 题）——全自动断言（工具命中对照流内提示、引用正则、联网“标记 + 真实链接”双证据、关键词子串，零 LLM judge）、分组成功率报告、断点续跑（改哪测哪）；配套 `scripts/` 维护工具（清缓存/断点管理）与一键重测脚本（停服务→启动→等就绪→清缓存→重跑）；基线 15/15（100%）
...
│   ├── eval_agent_questions.json    # Agent 评测集（15 题五分类）
...
> 在检索层评测（300 条 Recall@K/MRR）之上补齐 Agent 端到端评测：五分类 15 题（知识库/工具动作/联网/记忆/社交各 3 题）、全自动断言（工具命中/引用/联网双证据/关键词，零 LLM judge）、分组成功率与 P95 报告、断点续跑（改哪测哪）；配套维护工具（清缓存/断点管理）与一键重测脚本。**基线：15/15（100%），平均 ~14s。** 方法论沉淀：失败归因三分类（产品缺陷 / 评测题超纲 / 评测器误判）与“探针先行、现场取证”的排查流程。
